...
MSYS_NO_PATHCONV=1 docker compose exec -T mongo mongodump --db "$DB_NAME" --gzip --archive="$REMOTE"
...
MINIO_MOUNT="$MINIO_DATA"
if command -v cygpath >/dev/null 2>&1; then
  MINIO_MOUNT="$(cygpath -m "$MINIO_DATA")"
  export MSYS_NO_PATHCONV=1
fi
docker compose run --rm --no-deps \
  -v "${MINIO_MOUNT}:/backup-data" \
  --entrypoint sh \
  minio-init \
