...
import { TariffsPage } from './globals/TariffsPage'
...
const seedAdminEmail = 'demo@example.ru'
const seedAdminPassword = 'demo'
const enableDemoUser =
  process.env.NODE_ENV !== 'production' && !isNextBuildPhase && !isPayloadCliPhase
...
export default buildConfig({
  ...
  admin: {
    user: Users.slug,
    ...(enableDemoUser
      ? { autoLogin: { email: seedAdminEmail, password: seedAdminPassword } }
      : {}),
    importMap: {
  ...
  collections: [Users, Media, Plans],
  globals: [Header, Home, TariffsPage],
  plugins: [
    ...(useS3Storage
      ? [
          s3Storage({
  ...
  onInit: async (payload) => {
    ...
    payload.logger.info(`Seeded development CMS user: ${seedAdminEmail}`)
  },
})
